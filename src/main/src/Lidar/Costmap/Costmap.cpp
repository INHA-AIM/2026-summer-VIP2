...
// ========================================
// 1. kalman result TF 변환 (Lidar link -> base link)
// ========================================

bool transformKalmanToBaselink(const KalmanDetection& kd, float& out_x, float& out_y, float& out_yaw)
{
    try {
        geometry_msgs::TransformStamped tf =
            tfBuffer->lookupTransform("base_link", "Lidar3D", ros::Time(0), ros::Duration(0.05));
        ...
        out_x = pt_out.x;
        out_y = pt_out.y;
        out_yaw = kd.yaw + tf2::getYaw(tf.transform.rotation);

        return true;
    }
    catch (const exception& e) {
        ROS_ERROR("Kalman TF Error: %s", e.what());
        return false;
    }
}
...
// ========================================
// 추가 : fillGaussianCost() 함수
// ========================================

void fillGaussianCost(nav_msgs::OccupancyGrid& costmap,
                      float cx, float cy,
                      float length, float width,
                      float heading_theta)
{
    float sigma_x = length * 0.5f;
    float sigma_y = width  * 0.5f;
    float search_range = std::max(sigma_x, sigma_y) * 3.0f;

    float cos_h = cos(-heading_theta);
    float sin_h = sin(-heading_theta);

    int gx_min, gy_min, gx_max, gy_max;
    convertBaselinkToGrid(costmap, cx - search_range, cy - search_range, gx_min, gy_min);
    convertBaselinkToGrid(costmap, cx + search_range, cy + search_range, gx_max, gy_max);

    gx_min = std::max(0, gx_min);
    gy_min = std::max(0, gy_min);
    gx_max = std::min((int)costmap.info.width  - 1, gx_max);
    gy_max = std::min((int)costmap.info.height - 1, gy_max);

    for (int gy = gy_min; gy <= gy_max; ++gy) {
        for (int gx = gx_min; gx <= gx_max; ++gx) {

            float world_x = costmap.info.origin.position.x + (gx + 0.5f) * costmap.info.resolution;
            float world_y = costmap.info.origin.position.y + (gy + 0.5f) * costmap.info.resolution;
            ...
            float exponent = (local_x * local_x) / (2.0f * sigma_x * sigma_x)
                           + (local_y * local_y) / (2.0f * sigma_y * sigma_y);
            float gaussian = exp(-exponent);

            int8_t cost = static_cast<int8_t>(gaussian * 100.0f);
            if (cost <= 0) continue;

            int idx = convertGridToIndex(costmap, gx, gy);
            fillCostAtIndex(costmap, idx, cost);
        }
    }
}
...
void Costmap(const Lidar& st_Lidar, const ros::Time& stamp) 
{
    ...
    // ==================================
    // 추가: 칼만 결과로 가우시안 분포 cost 삽입
    // ==================================

    // for (const KalmanDetection& kd : st_Lidar.vec_kalman_clusters)
    // {
    //     float bl_x, bl_y, bl_yaw;
    //     if (!transformKalmanToBaselink(kd, bl_x, bl_y, bl_yaw)) continue;

    //     fillGaussianCost(state.costmap,
    //                      bl_x,
    //                      bl_y,
    //                      kd.length,
    //                      kd.width,
