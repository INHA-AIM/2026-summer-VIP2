...
{
  // 주변 환경 제거 ROI
  float lidar_range_xmin = -100.0f; // -50 -> -20
  float lidar_range_xmax = 100.0f;
  float lidar_range_ymin = -25.0f; // -25 -> -10
...
struct CostmapParams {
  float resolution = 0.05f; // 해상도 5cm 
  float width = 100.0f; //차량 전방 방향 100 -> 40
  float height = 50.0f; //차량 좌우 방향 50-> 20
  int unknown_cost = -1;
  int free_cost = 0;
...
struct KalmanDetection 
{
  ...
  // OBB length, width 정보 추가
  float length = 2.0f; // 차량 길이 평균값, 실제 클러스터 크기에 맞게 트랙 업데이트하면서 같이 저장
  float width = 2.0f;
};
...
struct Lidar
{
  ...
  vector<KalmanDetection> vec_kalman_clusters; // 이걸 LidarCluster 안에 넣어야 c
