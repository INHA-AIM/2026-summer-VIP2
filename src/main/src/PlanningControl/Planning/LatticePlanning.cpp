    ...
    for (auto& path : lattice_ctrl.candidates) {
        ...
        if (mode == 1) {
            ...
            path.cost = norm_obstacle * 0.40 +        
                        norm_lane * 0.25 +            
                        norm_curvature * 0.05 +       
                        norm_offset * 0.20 +          // 중심선 선호도 강화 (0.15 → 0.20)
...
void getTargetLocalPathIdx(LatticeControl& lattice_ctrl, double ld, int& out_idx) {
    ...
    for(int i = 0; i < lattice_ctrl.best_path.points.size(); ++i){
        ...
        if(local_dist > ld){
            target_idx = i;
            // Learning by cheating 관련 전역변수에 저장
            target_local_path_idx = target_idx;
            target_local_path_x = local_path_x;
            target_local_path_y = local_path_y;
            break;
        }
    }
    // 100~200개의 waypoint 중에서 ld 이상인 첫 번째 인덱스가 target_idx
    // 이떄 ld는 속도에 따라 증가, 예를 들어 5m, 10m, 15m, 20m 등으로 설정 가능 (속도 증가 시 더 먼 지점으로 목표 설정)
    // local_path_x, local_path_y 는 base_link 기준,
...
void getTargetSpeed(double max_curvature, double& out_target_vel, int lookahead_idx, int mode, const VehicleState& ego) {
    ...
    // const double K_CURVATURE_WEIGHT = 50.0;  // 조정 파라미터
    // const double MIN_SPEED_RATIO = 0.3;      // 최소 속도: target_vel * 0.3
    
    // if (max_curvature > 0.001) {
    //     // 곡률에 비례한 감속
    //     double speed_ratio = 1.0 - (K_CURVATURE_WEIGHT * max_curvature);
        
    //     // 최소/최대 속도 클램핑
    //     speed_ratio = std::max(MIN_SPEED_RATIO, std::min(1.0, speed_ratio));
        
    //     out_target_vel = target_vel * speed_ratio;
    //     ROS_WARN_THROTTLE(1.0, "[Speed] [Curve-DYNAMIC] %.1f km/h (κ=%.6f, ratio=%.2f)", 
    //                      out_target_vel * 3.6, max_curvature, speed_ratio);
    //     return;
    // }
