...
void VALatticePlanningProcess();  // Velocity-Aware Lattice Planning with MDS
void StaticLatticePlannerProcess(); // static LD Lattice Planning
void DynamicLDOnlyProcess();  // Baseline 1: Dynamic LD + Fixed Resolution
void MaxHighResProcess();  // Baseline 2: Fixed LD + Max Resolution
void generateOffsetGoalsMaxResolution(LatticeControl& lattice_ctrl);  // Helper for max resolution
void computeVelocityAdaptiveParams(double velocity_ms, int& num_offsets, double& lateral_offset_step);
...
void findStaticLookaheadGoal(const VehicleState& ego, int close_idx, LatticeControl& lattice_ctrl); // Static LD version
void generateOffsetGoals(LatticeControl& lattice_ctrl);
void generateConservativeOffsetGoals(LatticeControl& lattice_ctrl); // Conservative version for baseline
