...
// 실험 모드 설정
// ========================================
extern int EXPERIMENT_MODE;  // 0: Static, 1: VA, 2: VA+Fusion, 3: Others
...
struct PlannerParams {
    int num_offsets = 17; // 13 -> 9 // 횡방향 오프셋 후보의 개수
    double lateral_offset_step = 1.0; // 횡방향 오프셋 간의 간격 거리
    double sample_spacing = 0.2; 
    double lethal_cost_threshold = 90.0; // : 치명적 장애물 판단 임계값
    double vehicle_front_offset = 4.0; // 앞쪽 안전 검사 거리
};
...
// Learning by cheating 전역변수
extern double target_local_path_x;
extern double target_local_path_y;
extern int target_local_path_idx;  // 추적 waypoint 인덱스
